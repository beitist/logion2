from sqlalchemy.orm.attributes import flag_modified
from ..models import Segment
from .base import BaseWorkflow
from ..database import SessionLocal

def run_background_copy_source(project_id: str):
    db = SessionLocal()
    try:
        wf = CopySourceWorkflow(db, project_id)
        wf.run()
    except Exception as e:
        print(f"Copy Source Failed: {e}")
    finally:
        db.close()

class CopySourceWorkflow(BaseWorkflow):
    def run(self):
        try:
            self.log("Starting Copy Source to Target...")
            segments = self.db.query(Segment).filter(Segment.project_id == self.project_id).all()

            count = 0
            for seg in segments:
                seg.target_content = seg.source_content
                if not seg.status or seg.status == "new":
                    seg.status = "draft"

                count += 1

            self.db.commit()
            self.log(f"Copied source to target for {count} segments.")
            self.update_progress(100, status="ready")

        except Exception as e:
            self.fail(e)


class CopyCommentsWorkflow(BaseWorkflow):
    """Copy source content to target for comment segments only (preserve original comments)."""
    def run(self):
        try:
            self.log("Starting Copy Comments to Target...")
            segments = self.db.query(Segment).filter(Segment.project_id == self.project_id).all()

            count = 0
            for seg in segments:
                if (seg.metadata_json or {}).get("type") != "comment":
                    continue
                seg.target_content = seg.source_content
                if not seg.status or seg.status in ("new", "draft"):
                    seg.status = "translated"
                count += 1

            self.db.commit()
            self.log(f"Copied {count} comment segments to target.")
            self.update_progress(100, status="ready")

        except Exception as e:
            self.fail(e)
